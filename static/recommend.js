$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = 'eef164f6c47ab303bca44b6bedc1bc67';
    var title = $('.movie').val();
    if (title=="") {
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Sorry! The movie you requested is not in Vic3sax's database. Please check the spelling or try with some other movies"){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
